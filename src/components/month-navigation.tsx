...
export function MonthNavigation({ month, onChange }: { month: string; onChange: (month: string) => void }) {
  ...
  const latestMonth = shiftMonth(monthKey(), 1);
  ...
  return (
    <div className="flex items-center gap-1" aria-label="Budget month">
      ...
      <Button variant="ghost" size="icon" disabled={!canMoveForward} onClick={() => onChange(shiftMonth(month, 1))} aria-label="Next month" title={canMoveForward ? 'Next month' : 'Only one month ahead is available'}><ChevronRight /></Button>
    </div>
  );
}
