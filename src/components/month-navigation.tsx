...
export function MonthNavigation({ month, onChange }: { month: string; onChange: (month: string) => void }) {
  ...
  const canMoveForward = month < monthKey();
  return (
    <div className="flex items-center gap-1" aria-label="Budget month">
      ...
      <Button variant="ghost" size="icon" disabled={!canMoveForward} onClick={() => onChange(shiftMonth(month, 1))} aria-label="Next month" title={canMoveForward ? 'Next month' : 'Future months are not available'}><ChevronRight /></Button>
    </div>
  );
}
